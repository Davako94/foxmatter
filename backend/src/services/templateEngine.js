...
/**
 * Costruisce il context unendo i dati formattati dal formatterEngine.
 */
function buildStreamContext(stream, addonConfig) {
  // Calcolo fallback per grandezza se non presente
  let sizeBytes = stream.size;
  if (!sizeBytes && stream.title) {
    const sizeMatch = stream.title.match(/([\d.]+)\s*(GB|MB)/i);
    if (sizeMatch) sizeBytes = Math.round(parseFloat(sizeMatch[1]) * (sizeMatch[2].toUpperCase() === 'GB' ? 1e9 : 1e6));
  }

  return {
    stream: {
      ...stream, // Mantiene le proprietà arricchite dal formatter (quality, resolution, ecc)
      size: sizeBytes,
      addonName: addonConfig?.name || addonConfig?.slug || ''
    },
